...
describe('nextMessagesPage', () => {
  it('follows the cursor the server hands back while more remains', () => {
    expect(
      nextMessagesPage({ messages: [], more: true, cursor: '1700000000:m1' }, [
        undefined,
      ])
    ).toBe('1700000000:m1')
  })
  ...
  it('stops on the last page, and on a page that names no cursor', () => {
    expect(
      nextMessagesPage({ messages: [], more: false, cursor: null }, [undefined])
    ).toBeUndefined()
    expect(
      nextMessagesPage({ messages: [], more: true, cursor: null }, [undefined])
    ).toBeUndefined()
  })
