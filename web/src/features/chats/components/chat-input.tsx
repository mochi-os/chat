...
export const ChatInput = forwardRef<ChatInputHandle, ChatInputProps>(
  function ChatInput(
    ...
    const localMicHostRef = useRef<ReturnType<
      typeof createMicSessionHost
    > | null>(null)
    ...
    const pendingPlayableNotes = pendingAttachments.filter((a) =>
      Boolean(a.playable)
    )
    ...
    const toastMicError = (err: unknown) => {
      ...
      if (name === 'SecurityError') {
        if (typeof window !== 'undefined' && window.isSecureContext === false) {
          toast.error(
            t`Microphone access requires a secure context (HTTPS or localhost)`
          )
        } else if (isInShell()) {
          toast.error(
      ...
      >
        <div className='border-input bg-card focus-within:ring-ring flex w-full flex-col rounded-xl border focus-within:ring-1 focus-within:outline-hidden'>
          ...
          <div className='flex w-full items-end gap-2 px-4 py-2'>
            {isRecording ? (
              <div
                className='border-border/50 bg-muted/30 mb-0.5 flex w-full min-w-0 flex-1 items-center justify-end gap-3 rounded-xl border px-3 py-2'
                role='status'
                aria-live='polite'
              ...
              <>
                ...
                <label className='flex-1'>
                  <span className='sr-only'>
                    <Trans>Chat text box</Trans>
                  </span>
                  <MentionTextarea
                    ref={textareaRef}
                    rows={1}
                    placeholder={
                      replyTo ? t`Type your reply…` : t`Type your message…`
                    }
                    value={newMessage}
                    people={people}
                    onValueChange={(val) => {
                    ...
                    onMentionSelect={(person) => {
                      if (selectedMentions.some((m) => m.id === person.id))
                        return
                    ...
                    className='max-h-40 min-h-0 w-full resize-none overflow-y-auto rounded-none border-0 bg-transparent px-0 py-1.5 text-sm leading-5 shadow-none focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:outline-none'
                  />
                </label>
